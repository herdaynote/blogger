...
document.addEventListener('DOMContentLoaded', (a) => {
  ...
  ASMaxClick(b, d) && a(e).hide('fast'),
    ...
    a(window).addEventListener('beforeunload', function () {
      f && (ASMaxClick(b, d) ? a(e).hide('fast') : ASSetCookieAds(b, c))
    })
})

const ASSetCookie = (a, b, c) => {
  var d = new Date()
  d.setDate(d.getDate() + c)
  var e =
    escape(b) +
    (0 == c ? ';path=/' : '; expires=' + d.toUTCString()) +
    ';path=/'
  document.cookie = a + '=' + e
}
const ASGetCookie = (a) => {
...
const ASSetCookieAds = (a, b) => {
  var c = ASGetCookie(a)
  void 0 != c && '' != c
    ? ((ASTheCookieInt = parseInt(c) + 1),
      ASSetCookie(a, ASTheCookieInt.toString(), 0))
    : ASSetCookie(a, '1', b)
}
const ASMaxClick = (a, b) => {
  var c = ASGetCookie(a)
